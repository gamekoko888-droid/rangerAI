/**
 * event-buffer.mjs — Event buffering for WebSocket reconnect recovery
 * Extracted from server.mjs v63 during modular refactor.
 * 
 * Provides: EventBuffer class with disk persistence, periodic cleanup,
 * and O(1) session→msgId lookup.
 */
...
try { fs.mkdirSync(EVENT_BUFFER_DIR, { recursive: true }); } catch (e) { /* best-effort */ }
...
export class EventBuffer {
  ...
  startTask(msgId, sessionKey, userMessage) {
    ...
    this.sessionIndex.set(sessionKey, msgId);
  ...
  addEvent(msgId, event) {
    ...
    // BUG-11 fix: Increased event buffer limit to 5000
  ...
  completeTask(msgId) {
    ...
    // Memory cleared after 300s, disk after 1800s
    setTimeout(() => { this.buffers.delete(msgId); }, 300000);
    setTimeout(() => {
      try { fs.unlinkSync(path.join(EVENT_BUFFER_DIR, msgId + ".json")); } catch (e) { /* best-effort */ }
    }, 1800000);
  }
  ...
  _persist(msgId, task) {
    try {
      ...
      logger.info(`[${ts()}] [EventBuffer] Failed to persist: ${err.message}`);
    }
  }
  ...
  // Periodic cleanup for abandoned tasks
  startPeriodicCleanup(intervalMs = 600000) {
    this._cleanupTimer = setInterval(() => {
      ...
      if (cleaned > 0) {
        logger.info(`[${ts()}] [EventBuffer] Periodic cleanup: ${cleaned} items, ${this.buffers.size} active`);
      }
    }, intervalMs);
  }
