  ...
  onMessage: (msg) => {
    if (msg.type === "event" && (msg.event === "agent" || msg.event === "chat")) {
      ...
      if (typeof runId === "string" && runId.startsWith("announce:")) {
        ...
        // R54: Rate-limit UNREGISTERED event logging to avoid 205MB log bloat
        if (!globalThis._unregSeenRuns) globalThis._unregSeenRuns = new Set();
        if (!globalThis._unregSeenRuns.has(runId)) {
          globalThis._unregSeenRuns.add(runId);
          logger.info(`[${ts()}] [worker] [UNREGISTERED] Non-announce event: event=${msg.event} stream=${stream} runId=${runId} (first occurrence, further suppressed)`);
          // Prevent memory leak: cap the set size
          if (globalThis._unregSeenRuns.size > 500) {
            const iter = globalThis._unregSeenRuns.values();
            for (let i = 0; i < 250; i++) iter.next();
            const keep = new Set();
            for (const v of iter) keep.add(v);
            globalThis._unregSeenRuns = keep;
          }
        }
      }
    }
