...
| **3001** | OpenClaw Gateway HTTP | — | Gateway HTTP API 端口 |
| **3002** | rangerai-agent (API) | rangerai-agent.service | 后端 API 服务（纯 REST，无 WS） |
| **3003** | rangerai-acp | rangerai-acp.service | ACP Bridge（钉钉 + API Gateway） |
| **3004** | OpenClaw Gateway API | — | Gateway 兜底 API |
| **3005** | rangerai-ws (WS) | rangerai-ws.service | WebSocket + Worker + ChatOrchestrator |
| **18789** | OpenClaw Gateway WS | — | OpenClaw 核心引擎 WebSocket |

> **Iter-59 变更**：原 3001 端口的 HTTP API + WebSocket 混合服务已拆分为 3002 (API) + 3005 (WS) 双进程架构。
...
| URL 模式 | 目标 | 说明 |
|----------|------|------|
| `/ws` | 127.0.0.1:3005 | WebSocket 连接（ws-realtime 独立进程） |
...
└── worker/                        # ── Agent Worker 进程 ──
    ├── index.mjs                  # Worker 入口
    ├── user-message-handler.mjs   # 用户消息处理（薄胶水层，调用下方模块）
    ├── openclaw-handler.mjs       # OpenClaw 交互处理（薄胶水层）
    ├── message-router.mjs         # [v25.17] 路由决策（从 user-message-handler 拆出）
    ├── context-injector.mjs       # [v25.17] Context 注入 + 记忆召回（从 user-message-handler 拆出）
    ├── vision-handler.mjs         # 图片/视觉处理
    ├── task-planner.mjs           # 任务分解规划
    ├── wide-research.mjs          # Wide Research 触发逻辑
    ├── tool-orchestrator.mjs      # [v25.13] 工具编排 + 权限链（Rule 0.5）
    ├── tool-permission.mjs        # [v25.13] 工具权限定义（READONLY/HIGH/CRITICAL）
    ├── tool-tracker.mjs           # 工具调用追踪
    ├── tool-description.mjs       # 工具描述生成
    ├── context-window-manager.mjs # [v25.14] 上下文窗口管理
    ├── context-compressor.mjs     # [v25.14] 两级压缩（microCompact/autoCompact）
    ├── context-buffer.mjs         # 上下文缓冲
    ├── sub-agent-compactor.mjs    # [v25.14] 子 Agent 结果压缩
    ├── soul-loader.mjs            # [v25.15] SOUL.md 分层加载
    ├── skill-tool.mjs             # [v25.15] SkillTool 执行引擎
    ├── observability.mjs          # [v25.16] 可观测性埋点 + 指标聚合
    ├── run-tracker.mjs            # [v25.16] Run 全链路追踪 + 质量评分
    ...
    ├── intent-classifier.mjs      # 意图分类器
    ├── agent-config.mjs           # Agent 配置常量
    ├── human-approval.mjs         # 人工审批机制
    ├── model-handoff.mjs          # 模型切换
    ├── model-pricing.mjs          # 模型定价
    ├── output-manager.mjs         # 输出管理
    ├── usage-tracker.mjs          # 用量追踪
    ├── memory-manager.mjs         # 记忆管理
    ├── memory-extractor.mjs       # 记忆提取
    ├── adaptive-memory.mjs        # 自适应记忆
    ├── conversation-recall.mjs    # 对话召回
    ├── knowledge-injector.mjs     # 知识注入
    ├── jsonl-fallback.mjs         # JSONL 回退恢复
    ├── segmenter.mjs              # 分段器
    ├── smart-router.mjs           # 智能路由
    ├── llm-pre-classifier.mjs     # LLM 预分类
    ├── event-stream.mjs           # 事件流
    ├── visual-verifier.mjs        # 视觉验证
    ├── checkpoint-manager.mjs     # 检查点管理
    ├── autonomous-task-worker.mjs # 自主任务执行
    ├── task-progress-tracker.mjs  # 任务进度追踪
    ├── task-state-manager.mjs     # 任务状态管理
    ├── tools/                     # [v25.13] 统一工具注册表
    │   └── index.mjs              # 45+ 工具元数据
    ├── skills/                    # [v25.15] Skill 扩展系统
    │   ├── data-analysis/         # 数据分析 Skill
    │   ├── code-review/           # 代码审查 Skill
    │   └── server-ops/            # 服务器运维 Skill
    └── archive/                   # [v25.17] 归档（.bak / 死代码）
```

### 3.2 启动流程（双进程架构 — Iter-59）

系统采用 **API + WS 双进程** 架构，两个进程独立启动，通过 Redis IPC 通信：

#### 3.2.1 API 进程 (api-server.mjs, port 3002)
```
api-server.mjs (入口)
  │
  ├── lib/bootstrap.mjs          → 加载 .env 环境变量
  ├── lib/redis-ipc.mjs          → 初始化 Redis IPC 客户端
  ├── database.mjs.initDatabase() → 初始化数据库连接
  ├── lib/context-setup.mjs      → 组装 DI 容器
  │     ├── 注入 workerManagerProxy (Redis IPC 代理)
  │     ├── 注入 wsClientsProxy (Redis 通知代理)
  │     ├── 所有 API 模块 init()
  │     └── modules/http-router.mjs.init()
  │
  └── HTTP Server (port 3002, env: API_PORT)
        └── modules/http-router.mjs.handleRequest()
```

#### 3.2.2 WS 进程 (ws-realtime.mjs, port 3005)
```
ws-realtime.mjs (入口)
  │
  ├── lib/bootstrap.mjs          → 加载 .env 环境变量
  ├── lib/redis-ipc.mjs          → 初始化 Redis IPC 服务端
  ├── database.mjs.initDatabase() → 初始化数据库连接
  ├── lib/context-setup.mjs      → 组装 DI 容器
  │     ├── ChatOrchestrator(buildChatOrchestratorDeps)
  │     ├── modules/ws-handler.mjs.init()
  │     └── modules/worker-manager.mjs.init()
  │
  ├── WebSocket Server (port 3005, env: WS_PORT)
  │     └── modules/ws-handler.mjs.handleConnection()
  │
  ├── Worker Manager
  │     └── agent-worker.mjs (子进程)
  │
  └── Redis IPC 状态发布
        ├── rangerai:ws:status (每 5 秒)
        ├── rangerai:ws:tasks (活跃任务)
        └── rangerai:ipc:commands (命令监听)
```

#### 3.2.3 Redis IPC 通信协议
| 通道 | 方向 | 用途 |
|------|------|------|
| `rangerai:ws:status` | WS → Redis Hash | Worker 状态（每 5 秒更新） |
| `rangerai:ws:tasks` | WS → Redis Hash | 活跃任务列表 |
| `rangerai:ipc:commands` | API → WS (Pub/Sub) | 命令下发（restart/recover/cancel） |
| `rangerai:ipc:responses:{id}` | WS → API (Pub/Sub) | 请求-响应（browser 状态等） |
| `rangerai:ipc:ws-cleanup` | API → WS (Pub/Sub) | WS 连接清理通知 |
...
### 3.6 Worker 进程架构（归属 WS 进程）
Worker 由 **ws-realtime.mjs（WS 进程）** 管理，而非 API 进程。这确保了 Worker 的流式输出可以直接通过 WS 推送到前端，无需跨进程传递：
```
WS 进程 (ws-realtime.mjs, port 3005)
  │
...
API 进程 (api-server.mjs, port 3002) 通过 Redis IPC 间接访问 Worker 状态：
  ├── workerManagerProxy.status     → 读取 Redis Hash
  ├── workerManagerProxy.sendRequest() → Redis Pub/Sub 请求-响应
  └── wsClientsProxy.delete()       → Redis Pub/Sub 通知 WS 清理
```
...
### 7.5 systemd 服务管理（双进程架构）
```bash
# ── API 进程 (rangerai-agent → api-server.mjs, port 3002) ──
sudo systemctl status rangerai-agent
sudo /usr/local/bin/safe-restart-rangerai    # 推荐：带冷却期的安全重启
sudo journalctl -u rangerai-agent -f --no-pager
# 日志文件：/var/log/rangerai-agent.log, /var/log/rangerai-agent-error.log

# ── WS 进程 (rangerai-ws → ws-realtime.mjs, port 3005) ──
sudo systemctl status rangerai-ws
sudo systemctl restart rangerai-ws
sudo journalctl -u rangerai-ws -f --no-pager
# 日志文件：/var/log/rangerai-ws.log, /var/log/rangerai-ws-error.log

# ── 重启所有 RangerAI 服务 ──
sudo /usr/local/bin/safe-restart-rangerai && sudo systemctl restart rangerai-ws rangerai-acp rangerai-static

# ── 重载 Caddy 配置 ──
sudo caddy reload --config /etc/caddy/Caddyfile

# ── 环境变量覆盖 ──
# API 端口：/etc/systemd/system/rangerai-agent.service.d/port.conf → API_PORT=3002
# WS 端口：rangerai-ws.service 中 WS_PORT=3005
```
...
### Iter-59: HTTP/WS 进程分离
**日期：** 2026-03-15
**变更内容：**

**1. 架构拆分：单进程 → 双进程**
- `server.mjs` 拆分为 `api-server.mjs`（纯 REST API, port 3002）和 `ws-realtime.mjs`（WebSocket + Worker, port 3005）
- Worker 子进程归属 WS 进程，流式输出无需跨进程传递
- API 进程通过 Redis IPC 间接查询 Worker 状态

**2. 新增文件**
- `api-server.mjs`（~250 行）：API 进程入口
- `ws-realtime.mjs`（~310 行）：WS 进程入口
- `lib/redis-ipc.mjs`（~230 行）：Redis Pub/Sub 跨进程通信模块

**3. 基础设施变更**
- 新增 `rangerai-ws.service` systemd 服务
- Caddy `/ws` 路由从 3001 改为 3005
- `api-server.mjs` 使用 `API_PORT` 环境变量（避免与 .env 中 PORT 冲突）

**4. 路由模块适配**
- `admin-routes.mjs`：browser 状态和 breaker 重置改为 Redis IPC 请求-响应
- `static-routes.mjs`：gateway proxy 改为 Redis IPC 请求-响应

**5. 修复 Subpath Imports 双后缀 Bug**
- `#shared/message-types.mjs` → `#shared/message-types`（3 个文件）
...
## 十一、模块化自修复体系 (Self-Repair v4)

> **版本**: v4.0.0 (2026-03-15)
> **位置**: `/home/admin/.openclaw/workspace/skills/self-repair/`
> **原则**: 证据驱动 + 审批优先 + 安全第一

### 11.1 体系概述

Self-Repair v4 是 Ranger 的模块化自修复能力，允许 AI Agent 诊断问题、读取源码、生成修复方案，但**所有代码修改必须经过人工审批**。体系由 6 个独立 Shell 模块组成，通过 OpenClaw Skill 机制集成到 Ranger 的能力体系中。

### 11.2 模块清单

| 模块 | 脚本路径 | 行数 | 权限级别 | 职责 |
|------|----------|------|----------|------|
| 源码读取器 | `modules/source-reader.sh` | 217 | 只读 | 按模块名/路径安全读取源码 |
| 日志分析器 | `modules/log-analyzer.sh` | 189 | 只读 | 结构化日志分析 + 错误模式识别 |
| 方案生成器 | `modules/propose-fix.sh` | 243 | 写临时文件 | 生成 diff 格式修复方案 |
| 审批通知 | `modules/notify-approval.sh` | 143 | 通知 | 推送审批请求 + 状态管理 |
| 修复执行器 | `modules/apply-fix.sh` | 275 | 写+重启 | 执行已审批修复（备份→应用→验证→重启） |
| 回滚器 | `modules/rollback.sh` | 152 | 写+重启 | 回滚到备份版本 |

### 11.3 修复流程

```
用户报告 Bug / Ranger 自检发现异常
    │
    ▼
[source-reader] 读取相关源码文件（只读）
    │
    ▼
[log-analyzer] 分析错误日志，提取关键信息
    │
    ▼
[Ranger LLM] 综合分析，定位根因（OODA 循环）
    │ 输出：诊断报告
    ▼
[propose-fix] 生成修复方案（diff + 风险评估 + 回滚计划）
    │ 输出：FIX-{timestamp}-{hash}.md
    ▼
[notify-approval] 推送审批请求给 Joseph
    │
    ▼
[Joseph 审批] approve / reject
    │
    ▼ (approve)
[apply-fix] 备份 → 应用 patch → node --check → 重启 → 健康检查
    │
    ▼ (失败时)
[rollback] 恢复备份 → 重启服务
```
...
`registry/module-map.json` 维护了解耦后的完整模块映射，包含 8 个分类、40+ 个模块的路径和职责描述。Ranger 通过 `source-reader.sh` 使用此映射表定位源码文件。

### 11.5 安全机制

| 安全措施 | 实现方式 |
|----------|----------|
| 审批门控 | `apply-fix.sh` 检查 `[x] Joseph 审批通过` 标记 |
| 自动备份 | 每次修改前备份到 `/opt/rangerai-agent/backups/self-repair/` |
| 语法验证 | `node --check` 在应用 patch 后、重启前执行 |
| 自动回滚 | 语法检查失败或服务重启失败时自动恢复备份 |
| 操作审计 | 所有操作记录到 `repair-log.md` |
| 单次单修 | 一次只修复一个问题，避免交叉影响 |

### 11.6 与现有系统的关系

| 现有组件 | 关系 |
|----------|------|
| `worker/self-healer.mjs` | 流式恢复（工具摘要/截断续写），不涉及代码修改 |
| `remediation-engine.mjs` | 基础设施修复（进程重启/chromium 回收），Self-Repair v4 处理代码级修复 |
| `alert-manager.mjs` | 告警通道，Self-Repair v4 的 notify-approval 是独立的审批通道 |
| `self-diagnosis` Skill | 全面自检，发现问题后切换到 self-repair 处理 |

### 11.7 文件结构

```
/home/admin/.openclaw/workspace/skills/self-repair/
├── SKILL.md                          # 主入口文档（274 行）
├── modules/
│   ├── source-reader.sh              # 源码读取器
│   ├── log-analyzer.sh               # 日志分析器
│   ├── propose-fix.sh                # 方案生成器
│   ├── notify-approval.sh            # 审批通知
│   ├── apply-fix.sh                  # 修复执行器
│   └── rollback.sh                   # 回滚器
├── registry/
│   └── module-map.json               # 架构映射表（97 行）
└── history/                          # 已完成的修复方案归档
```
